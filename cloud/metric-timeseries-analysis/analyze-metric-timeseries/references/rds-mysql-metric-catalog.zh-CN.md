# RDS for MySQL 指标 ID 目录

本 reference 用于为 RDS for MySQL 实例或数据库代理选择准确的 CES
`metric.metric_name`。指标名称保留华为云官方中文表述。

来源：华为云
[RDS for MySQL 支持的监控指标](https://support.huaweicloud.com/usermanual-rds-mysql/rds_06_0001.html)。
源页面更新时间为 2026-06-24，本 reference 核对日期为 2026-07-31。

## 目录

- [选择规则](#选择规则)
- [RDS for MySQL 实例指标](#rds-for-mysql-实例指标)
- [数据库代理指标](#数据库代理指标)

## 选择规则

- `metric.metric_name` 填写指标 ID，不填写指标名称。
- 单机和主备实例使用 namespace `SYS.RDS`，dimension 为
  `rds_cluster_id`。
- 集群版实例使用 namespace `SYS.RDS_MYSQL_CLUSTER`，dimension 按顺序为
  `rds_cluster_id,rds_instance_id`。
- 集群版不支持 `rds_threadpool_waiting_threads`。
- 数据库代理使用 namespace `SYS.DBPROXY`，dimension 按顺序为
  `dbproxy_instance_id,dbproxy_node_id`。
- 某项指标在特定区域、引擎版本或部署形态下不可用时，以官方源页面当前说明为准。
